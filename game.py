...
def tick(keys):
    ...
    if game_started == False:
        ...
        title = gamebox.from_text(400, 200, "SpaceBreaker", "Arial", 100, "green")
        camera.draw(title)
        subtitle = gamebox.from_text(400, 260, "brought to you by Samarth Kishor and Andrew McCullough", "Arial", 30, "green")
        camera.draw(subtitle)
        subsubtitle = gamebox.from_text(400, 300, "sk4gz // asm4wm", "Arial", 25, "green")
        ...
        for each in controls:
            h += 1
            camera.draw(
                gamebox.from_text(400, 340 + 30 * h, each, "Arial", 25, "yellow")
            )

        for each in instructions:
            h += 1
            camera.draw(
                gamebox.from_text(400, 360 + 30 * h, each, "Arial", 25, "yellow")
            )

        if pygame.K_SPACE in keys:
            game_started = True

    elif game_started == True and game_active == False and game_over == False:
