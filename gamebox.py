...
def _image(key, flip=False, w=0, h=0, angle=0):
    ...
    if w == 0 and h == 0:
        ...
        _known_images[(key, flip, tmp.get_width(), tmp.get_height(),
                       angle)] = ans
    return ans
...
def from_text(x, y, text, fontname, fontsize, color, bold=False, italic=False):
    ...
    if font is None:
        sys.stderr.write("ERROR: no font named " + fontname +
...
class Camera(object):
    ...
    def __init__(self, width, height, full_screen=False):
        ...
        if full_screen:
            self.__dict__['_surface'] = pygame.display.set_mode(
    ...
    def draw(self, thing, *args):
        ...
        if isinstance(thing, SpriteBox):
            ...
            try:
                ...
                self._surface.blit(
                    thing,
                    [x - thing.get_width() / 2, y - thing.get_height() / 2])
            except e:
            ...
            try:
                ...
                if type(color) is str: color = pygame.Color(color)
                self.draw(
                    pygame.font.Font(font, size).render(thing, True, color),
                    *args[3:])
            except e:
                raise Exception(
                    "Wrong arguments; try .draw(text, fontName, fontSize, color, [x,y])"
                )
        else:
    ...
    def __getattr__(self, name):
        ...
        x, y, w, h = self._x, self._y, self._surface.get_width(
        ...
        if name == 'mouse':
            return pygame.mouse.get_pos()[0] + self._x, pygame.mouse.get_pos(
            )[1] + self._y
    ...
    def __setattr__(self, name, value):
        ...
        if name == 'left': self._x = value
            ...
            self._x, self._y = value[0] - w, value[1] - h
                ...
                'width', 'height', 'size', 'mouse', 'mousex', 'mousey',
                'mouseclick'
        ]:
    ...
    def __str__(self):
        return '%dx%d Camera centered at %d,%d' % (self.width, self.height,
                                                   self.x, self.y)
...
class SpriteBox(object):
    ...
    def __init__(self, x, y, image, color, w=None, h=None):
        ...
        if image is not None:
            ...
            if w is None or h is None:
                raise Exception("must supply size of color box")
    ...
    def __str__(self):
        return '%dx%d SpriteBox centered at %d,%d' % (self._w, self._h, self.x,
                                                      self.y)

    def copy_at(self, newx, newy):
        '''Make a new SpriteBox just like this one but at the given location instead of here'''
        return SpriteBox(newx, newy, self._image, self._color, self._w,
                         self._h)
    ...
    def scale_by(self, multiplier):
        ...
        if self.__dict__['_key'] is None:
            ...
            key = self.__dict__['_key']
            self._set_key(key[0], key[1], key[2] * multiplier,
                          key[3] * multiplier, key[4])
    ...
    def draw(self, surface):
        ...
        if isinstance(surface, Camera):
            if self.__dict__['_color'] is not None:
                ...
                surface._surface.blit(
                    self._image,
                    [self.left - surface._x, self.top - surface._y])
        else:
            if self.__dict__['_color'] is not None:
...
def pause():
    ...
    if not _timeron:
        raise Exception(
            "Cannot pause a timer before calling timer_loop(fps, callback)")
    pygame.time.set_timer(pygame.USEREVENT, 0)
...
if __name__ == "__main__":
    ...
    smurfs = load_sprite_sheet(
        "http://www.flashpulse.com/moho/smurf_sprite.PNG", 4, 4)
